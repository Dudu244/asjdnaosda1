...
local b = {
	main_toggle = ui.new_checkbox("AA", "Fake lag", "Enable Kvasibo"),
	mAAEPeekFreestand = ui.new_hotkey("AA", "Fake lag", "Flick"),
	mAAEPeekInvert = ui.new_hotkey("AA", "Fake lag", "Inverter"),
	EPeekCombo = ui.new_combobox("AA", "Fake lag", "Style", "Rage", "Legit")
}
local c = require "gamesense/surface"
local d = c.draw_text
local e = c.create_font("Verdana", 12, 700, {0x200})
local f = c.create_font("Verdana", 15, 700, {0x200})
local function g()
...
local function i()
	...
	if p and ui.get(a.DoubleTap[2]) then
		renderer.rectangle(m[1] - 730, m[2] + 30, 74, 7, 0, 0, 0, 200)
		renderer.gradient(m[1] - 730, m[2] + 31, j * 4, 5, 180, 60, 120, 255, 30, 30, 30, 255, false)
		d(m[1] - 730, m[2] + 34, o[1], o[2], o[3], 255, e, "Kvasibo.AA : " .. tostring(j))
		if ui.get(b.mAAEPeekInvert) then
			d(m[1] - 48, m[2] + 5, o[2], o[2], o[3], 255, f, "⮜")
		end
		...
		if j == 5 and ui.get(a.DoubleTap[2]) and p or j == 10 and ui.get(a.DoubleTap[2]) and p then
			...
			if ui.get(b.EPeekCombo) == "Rage" then
				ui.set(a.yaw[2], "0")
