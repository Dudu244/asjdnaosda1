...
client.set_event_callback("aim_miss", function(e)
	...
	if compare(dtype, script.type[1]) then
		print(string.format(" [ miss ] ent: %s | hitbox: %s | dmg: %i | hc: %s | bt: %i | reason: %s",
			name,
			hgroup,
			on_fire_data.damage,
			hitchance,
	...
	if compare(dtype, script.type[2]) then
		print_chat(string.format(" \x06[ miss ] \x08ent: \x07%s\x08 | hitbox: \x10%s\x08 | dmg: \x05%i\x08 | hc: \x04%s\x08 | bt: \x04%i\x08 | reason: \x07%s", 
		name,
		hgroup,
		on_fire_data.damage,
		hitchance,
...
client.set_event_callback("aim_hit", function(e)
	...
	if compare(dtype, script.type[1]) then
		print(string.format(" [ hit ] ent: %s | sv hitbox: %s | cl hitbox: %s | dmg: %i | remain: %ihp | hc: %s | bt: %i",
			name,
			hgroup,
			aimed_hgroup,
			e.damage,
	...
	if compare(dtype, script.type[2]) then
		...
		print_chat(string.format(" \x06[ hit ] \x08ent: \x07%s\x08 | sv hitbox:  \x04%s\x08 | cl hitbox: \x09%s\x08 | dmg: \x05%i\x08 | remain: \x05%i\x08hp\x08 | hc: \x04%s\x08 | bt: \x04%i",
		name,
		hgroup,
		aimed_hgroup,
		e.damage,
